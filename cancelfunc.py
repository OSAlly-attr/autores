...
def main(driver):
    ...
    id_send_keys(driver, registered_number_text_field, account[0]) 
    id_send_keys(driver, password_text_field, password[0])
    ...
    xpath_send_keys(driver, purpose_of_use, sport)
    time.sleep(1)
    ...
    xpath_send_keys(driver, facility_name_text_field, school)
    xpath_click2(driver, facility_item)
    xpath_click(driver,"/html/body/div/div/div[3]/div/main/div[1]/div[1]/div[1]/div[1]/div[1]/h2")
    while(1): 
        if xpath_get(driver, facility_pannel) == school[0]:
            break
        else:
            time.sleep(0.5)
    ...
    xpath_send_keys(driver, date_of_use, date)
    # 空き施設検索
    ...
    if len(driver.find_elements(By.XPATH, tutorial_skip_button))!=0:
        xpath_click(driver, tutorial_skip_button)
    while(1):
        elements = driver.find_elements(By.CLASS_NAME, time_slot_class_c)
        if len(elements)!=0:
            elements_list = []
            for elm in elements:
                elm_text = elm.get_attribute('title')
                if '利用可能' in elm_text:
                    ...
                    for i in range(len(elm_text)):
                        if elm_text[i]=="時":
                            elm_text = int(elm_text[0:i])
                            break
                    if start_time_list[int(jikan)] <= elm_text and elm_text < start_time_list[int(jikan)+1]:
                        elements_list.append(elm)
...
time_slot_class_c = "AvailabilityFrameSet_frame_content"
# 確認
